...
public class SortingHandler {
	...
	public static void checkForConflicts(){
		AlchemicalBrewing.logger.info("Checking for potion id conflicts");
		if (Potion.potionTypes.length < Config.maxPotionID)
			AlchemicalBrewing.logger.error("[Warning] It seems another mod has reduced the potion array size from 2048 to " + Potion.potionTypes.length);
		...
		//Print remap options
		AlchemicalBrewing.logger.info("##########################################################################");
		AlchemicalBrewing.logger.info("Found the following potion id conflicts");
		System.out.println();
		for (String s : remapOptions) AlchemicalBrewing.logger.info(s);
		System.out.println();
		...
		try
		{
			File file = new File(AlchemicalBrewing.configFolder, "/PotionIdConflicts.txt");
			...
			AlchemicalBrewing.logger.info("This list has been saved to " + file.getAbsolutePath());
			AlchemicalBrewing.logger.info("##########################################################################");
		}
		catch (IOException e) {
			e.printStackTrace();
	...
	public static String getUName(Potion potion) {
		return potion.getClass().getName() + ":" + potion.getName();
	}
}
